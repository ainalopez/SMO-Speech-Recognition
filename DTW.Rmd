...
The minimum distances correspond to the the same word all the times. This shows that dynamic time warping has a good performance when dealing with speech recognition. Unfortunately, we don't have enough data to claim that it will always work.  
\
...
Additionally, the DTW algorithm is very fast with our data. Approximately it takes 0.09 seconds.  
\
...
More Robust -> more template Data

It works for both male female

Not enough data to make universal claims 

DTW fast and good

Improve it, maybe better speech processing techniques -> to deal with sentences 

Saddly we haven't found a way to input real voice, but would be interesting as a application. 
