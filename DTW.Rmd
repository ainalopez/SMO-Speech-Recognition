...
This condition guarantees that the index does not jump over time, meaning the features are not repeated in the alingment. 
...
This condition implies that the alingment path starts at the bottom left and ends at the top right. This is useful to guarantee that the alingment does not consider only one of the sequences. 
...
This condition forces the alingment path to be close to the diagonal. This ensures that the path does not skip features or get stuck at some features.
...
If $P=0$ then there are no restrictions to the slope of the warping function. If $P=\infty$ then the warping function is restricted to the diagonal line $i=j$.

This condition prevents the short parts of the sequences matching to long parts of the sequence. 
