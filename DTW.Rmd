...
### Derivative DTW
Derivative DTW [1] proposes replacement of the value of each data point with its estimated local first derivation in the process of dynamic time warping. The improvement of the classic DTW is shown in the figure below, as after creating 1) two artificial signals, and identifying 2) the intuitive feature to feature warping alignment, the alignment produced by classic DTW and the alignment produced by DDTW are sections 3) and 4) respectively.  
![DDTW](./Figures/ddtw.png)

### Feature Based DTW and Adaptive Feature Based DTW

During various implementations of DTW, it was noticed that the value or the deviation of a point may not reflect the position of this point in global or local trends of the sequence. Along the further research on the issue, Feature Based DTW and Adaptive Feature Based DTW by Xie and Wiltgen [2] are introduced.

Feature Based DTW algorithm [2] works in the following way: first defines a global feature and a local feature for each point in a time series sequence, then dynamically aligns two time series sequences based on both the global features and local features of each points in the sequences.
