...
Flexible DTW [2] method is developed for measuring the similarity of two time series sequences. The Flexible DTW algorithm adds an additional score as the reward for the contiguously long one-to-one fragment, i.e. makes the lengths of contiguous one-to-one fragments as long as possible.
However, the most successful implementation of the algorithm does not stand alone and the experimental results show that by combining the Flexible DTW, DTW and Derivative DTW methods to form a classifier ensembled with the voting scheme, there is a lesser average error rate than that of each individual method.

### Feature Based DTW and Adaptive Feature Based DTW

The results of DTW modifications showed, however, that the value or the deviation of a point may not reflect the position of this point in global or local trends of the sequence. Upon further research of the issue, Feature Based DTW and Adaptive Feature Based DTW by Xie and Wiltgen [3] are introduced.

Feature Based DTW algorithm works in the following way: first the algorithm defines a global and local feature for each point in a time series sequence, then dynamically aligns the two time series sequences based on both the global and local features of each points in the sequences.
...
DTW is often used in handwriting and online signature matching, sign language and gestures recognition, data mining and time series clustering, computer vision and computer animation, surveillance, protein sequence alignment and chemical engineering, and signal processing.  
...
An extended approach is to transform the sound waves into the frequency domain and delete the unnecessary frequencies while extracting some coefficients ([5], [6]). In this project, we tried with two different coefficients: Mel Frequency Cepstral Coefficients (MFCC) and Linear Prediction Coefficient (LPC). Unfortunately, we didn't obtained good results with LPC and we decided to use only MFCC.  
\
\
...
The minimum distances correspond to the the same word all the times. This shows that dynamic time warping performs well when dealing with speech recognition. Unfortunately, we don't have enough data to claim that it will always work.  
\
In the figure below we can observe some warping paths. 

![](Figures\WarpingPath.png)
*Figure 3. Warping Paths of one of our examples. Red lines represent the warping path between two different words and green lines represent the warping path between same words.*

Additionally, the DTW algorithm is very fast with our data, taking approximately 0.09 seconds on the data we gave it.  
\
...
We found that our algorithm works for both male and female voices. Thanks to MFCC, we are able to extract the important frequencies and eliminate the others, creating a recognizer robust to gender. In the near future we would like more template data to make our algorithm more robust. We would like to test our algorithm on more voices, but with the limited data we had, we are not able to make universal claims at this time. We found our dynamic time warping function performed fast and accurately, however there is room for improvement. Our algorithm works well on individual words, however it remains untested on sentences. From R, we were not able to import voices in real time, however this would add a unique capability to our algorithm. 
...
[2] Hsu, C.-J., Huang, K.-S., Yang, C.-B. & Guo Y.-P. [Flexible Dynamic Time Warping for Time Series Classification](http://www.sciencedirect.com/science/article/pii/S1877050915012521)

[3] Xie, Y. & Wiltgen, B. [Adaptive Feature Based Dynamic Time Warping](http://paper.ijcsns.org/07_book/201001/20100135.pdf)

[4] Bahlmann C. & Burkhardt, H. [The Writer Independent Online Handwriting Recognition System frog on hand and Cluster Generative Statistical Dynamic Time Warping](http://lmb.informatik.uni-freiburg.de/people/bahlmann/data/ba_bu_tpami04.pdf)
